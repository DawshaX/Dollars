...
def frames_stats(path, fps: int = 4, w: int = 240) -> dict:
    """يفكّ الفيديو لكادرات صغيرة ويحسب الحركة والإضاءة (بالصورة الحقيقية)."""
    r = subprocess.run([FF, "-hide_banner", "-loglevel", "error", "-i", str(path),
                        "-vf", f"fps={fps},scale={w}:-2", "-f", "rawvideo", "-pix_fmt", "gray",
                        "-"], capture_output=True)
    buf = np.frombuffer(r.stdout, dtype=np.uint8)
    if buf.size == 0:
        return {"frames": 0}
    # نعرف الطول من عدد البكسلات: نجرّب أطوال مختلفة
    for h_guess in range(1, 2000):
        if buf.size % (w * h_guess) == 0 and buf.size // (w * h_guess) > 1:
            h = h_guess
            break
    else:
        return {"frames": 0}
    arr = buf.reshape(-1, h, w).astype(np.float32) / 255.0
    ...
    return {"frames": int(len(arr)), "fps": fps,
...
def inspect(path, expect_seconds: float | None = None, kind: str = "short",
    ...
    out["checks"].append(("مفيش ثبات طويل", _n(fs.get("longest_still_s"), 9) < 2.5,
                          f"أطول ثبات {_n(fs.get('longest_still_s')):.1f} ثانية"))
