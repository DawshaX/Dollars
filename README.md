...
## 🎨 هوية القناة
```
assets/brand/  →  أفاتار 800×800 · بانر 2560×1440 · علامة مائية · شاشة نهاية · أيقونة شورتس · قالب غلاف
docs/BRANDING.md  →  الاسم · الوصف · الكلمات المفتاحية · تظبيط يوتيوب استوديو خطوة بخطوة
```
كلها مرسومة بالكود: `python tools/make_branding.py`

## 🏭 المصنع (بيشتغل نفسه)
| الأمر | بيعمل إيه |
|---|---|
| `python engine/factory.py --hourly` | شورت الساعة (مونتاج كامل + غلاف + بيانات) وينشره |
| `python engine/factory.py --daily` | الطويلة (3/8/10 ساعات) + قصة بلا كلام |
| `python engine/factory.py --status` | تقرير المصنع |
| `python engine/factory.py --render-queue` | بعد ربط القناة: يعيد إنتاج كل حاجة في الطابور بنفس البذرة **وينشرها** |

سير العمل: `factory-hour.yml` (كل ساعة) · `factory-daily.yml` (يومي) · `factory-publish-queue.yml` (نشر الطابور).
لو مفيش توكن للقناة، المصنع **مش بيفشل** — بيحفظ الوصفة الكاملة (السلوت + البذرة) في الطابور ويكمّل.


## 📊 الوكيل المراقب (بيطوّر نفسه بالأرقام)
`python engine/analytics.py --sync` → يجيب مشاهدات/لايكات/تعليقات كل فيديو من يوتيوب (مفتاح `YOUTUBE_API_KEY`)
ويسلّمها للعقل، والعقل يعدّل أوزانه ويغيّر الخطة (**النوع · الساعة · الطول · المشهد**) في اليوم اللي بعده.
`python engine/analytics.py --report` → تقرير: أفضل نوع/ساعة/طول فعلي.


## 📦 المخزون المحلي + 🔎 المرجع البصري
```
engine/stock.py   →  60 مؤثر صوتي · 6 أنماط موسيقى · 36 ملصق · 6 ميمز · 10 إضافات بصرية · 12 مشهد حيّ
engine/fx.py      →  إضافات بصرية معادلات (ضوء · هالة · غبار · بوكيه · بلوم · لمعات) — مخزون لا نهائي
engine/music.py   →  موسيقى من صنعنا (6 أنماط · حلقات مقفولة بلا قطع)
engine/refs.py    →  مرجع بصري (Pinterest + Openverse/Pixabay/Pexels) + أرقام السوق من يوتيوب
```
التفاصيل: `docs/STOCK.md` · `docs/REFS.md` · `docs/GITHUB_TOOLS.md`
